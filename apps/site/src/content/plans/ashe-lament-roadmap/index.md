...
我会把《亚舍拉挽歌》的后续更新拆成几轮验证：

- `0.2 引导版`：重做开场目标、第一次地下入口、第一次返回和第一次资源提示，只关注玩家能否完成第一轮循环。
- `0.3 资源版`：把灵能、遗骨、加时器和捷径分阶段讲清楚，让玩家知道每种资源改变了什么。
- `0.4 反馈版`：补失败原因提示、灵能消耗来源提示和捷径建造前后对比，让玩家更容易知道下一次该怎么改。
- `0.5 节奏版`：在理解门槛降低后，再调整早期路线、地下时间压力和结局阈值。

这个顺序刻意避开“先加新系统”。现在最重要的不是让游戏更复杂，而是让玩家在前三分钟先接住现有复杂度。
