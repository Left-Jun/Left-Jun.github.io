...
我会把后续更新拆成几个更小的版本，而不是一次性重做：

- `0.2 手感版`：只处理移动、跳跃、冲刺、镜头和死亡复活，不新增面具，先保证现有版本更稳。
- `0.3 教学版`：重做前 1 分钟，把三种面具拆成三个最小任务，让玩家不用长文字也能理解规则。
- `0.4 面具原型版`：在独立测试关里验证新面具，先判断它是否带来新的关卡读法，再决定是否进入主线关卡。
- `0.5 内容版`：在系统稳定后再扩展关卡、剧情表现和声音反馈，避免内容建立在不稳定手感上。

这个顺序的核心是先修“玩家每天都会碰到的问题”，再加“只有新内容才会触发的问题”。如果手感和镜头没有稳定，新增面具只会放大原本的操作问题。
